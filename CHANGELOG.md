...
### Added
- `Column.dir`: `'ltr' | 'rtl' | 'auto'`, applied as the native HTML `dir` attribute on both a column's header and body cells — for right-to-left content (Arabic, Hebrew, …) inside an otherwise left-to-right table, independent of `align`. New Storybook story: `ColumnTextDirection`. `Table.fixtures.tsx`'s `UserRow`/`makeRows` gained a `nameArabic` field (sample RTL-script data) to demonstrate it.
- `FilterConfig.position`: `'toolbar'` (default, unchanged behavior) or `'header'` — renders each filterable column's control in its own cell directly under that column's header, Excel/AG-Grid style, instead of grouped in the toolbar. The toolbar still surfaces a "Clear filters" button when filters are active and positioned in the header, since there's no natural place for it there. New Storybook story: `FiltersInHeader`. `FilterControl` (the per-column filter-rendering logic) is now extracted into its own module (`FilterControl.tsx`) so both `TableToolbar` and `TableHeader` can share it.
